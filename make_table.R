...
setwd(mother_dir)
col_names <- c("Truth", "Model", "Bounds",
               "Intercept", "X1", "X2",
...
for(filename in filenames){
  ...
  row <- c(row, sprintf("%.2f, %.2f%%", signif(sqrt(avg_bvm[3]),3),
  ...
  row <- c(row, sprintf("%.0f+%.0f=%.0f,%.0f%%",sw_bvm[1],sw_bvm[2],sw_bvm[3],
                        sw_cover))
  ...
  row <- c(row, sprintf("%.0f+%.0f=%.0f,%.0f%%",sy_bvm[1],sy_bvm[2],sy_bvm[3],
                        sy_cover))
  df <- rbind(df, row)
}
