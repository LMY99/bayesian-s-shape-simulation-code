...
est <- apply(points,1,function(x) c(mean(x),HDInterval::hdi(x,0.75)))
...
sigmay_repeat[di,2:3] <- HDInterval::hdi(stan.array$sigmaerror,0.75)
...
sigmaw_repeat[di,2:3] <- HDInterval::hdi(stan.array$sigmarandom,0.75)
...
turning[di,1:3] <- c(mean(stan.array$lpos),
                  HDInterval::hdi(stan.array$lpos,0.75))
...
amp[di,] <- c(mean(stan.array$lamp),
              HDInterval::hdi(stan.array$lamp,0.75))
true_scales[di] <- 5
scales[di,] <- c(mean(stan.array$lscale),
              HDInterval::hdi(stan.array$lscale,0.75))
}
